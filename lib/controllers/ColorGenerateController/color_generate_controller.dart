  ...
  Future<void> getColorNameRepo({required String hexColorCode}) async {
    ...
    var bodyData = {
      "contents": [
        {
          "parts": [
            {"text": "Hex Color Code:$hexColorCode, give the Color name from the hex color code"}
          ]
        }
      ]
    };
    try {
      var response = await http.post(url, body: jsonEncode(bodyData), headers: headers).timeout(const Duration(seconds: 30));
      ...
      if (response.statusCode == 200) {
        ...
        colorName.value = geminiResponseModel?.candidates?[0].content?.parts?[0].text ?? "Not Found";
      } else {
        Get.snackbar("Invalid value".tr, "Something went wrong, Try again!".tr,);
      }
      isLoading.value = false;

    } catch (error) {

      Get.snackbar("Invalid value".tr, "Something went wrong".tr,);
